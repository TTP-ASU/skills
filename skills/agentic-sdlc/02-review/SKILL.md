...
3. **Check against phase goals:**
   ...
   - **PRD (what and why):** §1–4 business context and goals; §5 scope (in + out); §6 dependencies; §7 FRs grouped by theme with persona anchors; §8 NFRs; §9 user/process flows; §10 success metrics; §12 open questions — each with **owner**, **status**, and trace to a single source of truth (often `open-questions.md`). Do not mix Brief, PRD, and TRD: keep architecture and implementation out of the PRD — that belongs in the TRD.
   - **TRD (how):** §2 architecture aligned to PRD scope; §3 system interactions reference PRD flow IDs; §10 backlog stories trace to FRs; §12 FR coverage audit shows no gaps. Do not restate business “why” here — reference the PRD instead.
