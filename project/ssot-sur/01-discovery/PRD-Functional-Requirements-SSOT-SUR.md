...
**Last updated:** 2026-03-20 (EA Support (2) + Part Rel Mgmt meetings incorporated; D2 resolved; I2 resolved; J questions added)  
**Status:** Draft  
...
> **Prerequisite gate:** No engineering sprint for Q1 milestones should be groomed until `[ARCH]` blockers A1, A2, D1, D3, D4, and E2 are resolved. **D2 resolved 2026-03-19/20** (lead times confirmed in DAX API). Target: remaining blockers resolved at architecture working session (est. week of 2026-03-24).
...
| A4 [ARCH] | Can job-type exclusions (currently SKU-level in SB) be modeled at equipment-type level in F&O, or must they stay SKU-level? | Robbie Carter / DAX | Open | **EA Support (2) 2026-03-20 direction:** DAX is the data container for job type exclusion config (stored as part attribute via API + UI for Robbie-level users). SB makes all eligibility decisions. DAX applies no logic. SKU-level vs equipment-type-level still open — architecture team to confirm. |
...
| I1 [ARCH] | BOM Lookup architecture: embed BOM lookup inside EIAA (single call) — OR — separate BOM service that ServiceBench calls first? Shown as "PRISM BOM?" open question in architecture diagram. | Ankit / DAPI Team / Integration Team | Open | **EA Support (2) 2026-03-20:** "God API" approach (Option 2 — BOM inside EIAA) discussed in depth. Returns: BOM explosion + replacement matrix priorities (1-3) + availability by location + JIT flags + lead times + job type exclusions. Matt confirmed as preferred direction. API contract working session scheduled for week of 2026-03-23 (Wed/Thu). Formal decision expected then. |
| I2 [ARCH] | AOP JIT reservation: is a reservation required when inventory availability is false for AOP JIT scenarios? | Avengers / DAX Arch | **Resolved** | **Part Rel Mgmt 2026-03-19:** No reservation needed for out-of-stock JIT-enabled parts. Aligns with DDD open item 33 direction. EP4-S1 and EP5-S1 blocker on I2 cleared. |
...
**Risks:** Q1 at risk; TELUS Plan B is a technology concession for one client; AOP UI path (MFE vs WMA) must be decided before grooming; 13 `[ARCH]` blockers remain open (D2 + I2 resolved this week) pending architecture review (est. week of 2026-03-24); Product 360 / Product Intelligence team overlap (J1) needs boundary clarification with Mika; Remote Tech identity model (J2) not yet scoped. **Architecture direction confirmed 2026-03-20:** ServiceBench stays in the middle (Horizon → SB → DAX); DAX is data provider, SB is adjudicator; "God API" (BOM inside EIAA) preferred approach for I1.
